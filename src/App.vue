...
<template>
  ...
  <WriteText header="Background" :text="pageText.background" :active="writeElement === 0" :writingDone="() => next(1)" />
  <WriteText header="Contact" :text="pageText.contact" :links="pageText.links" :active="writeElement === 1"
    :writingDone="() => next(2)" />
  <div class="fade-in" v-if="writeElement >= 2"
    style="display: flex; flex-direction: column; justify-content: center; padding-bottom: 10rem;">
    <div v-for="project, index in pageText.projects" class="project">
      <WriteText :key="project.title + index" :header="'Project ' + (index + 1) + ' - ' + project.title"
        ...
        :tags="project.technologies" :writingDone="() => next(3 + index)" />
    </div>
  </div>
</template>
